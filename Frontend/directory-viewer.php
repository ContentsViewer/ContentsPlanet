...
if (!empty($subDirs)) {
    ...
    foreach ($subDirs as $subDir) {
        $body .= '<a class="directory" href="' . CVUtils\CreateDirectoryHREF($subDir, $vars['language']) . '">';
...
if (!empty($contents)) {
    ...
    foreach ($contents as $contentPath) {
        ...
        if ($content->SetContent('.' . RemoveExtention($contentPath))) {
            ...
            $body .= CVUtils\CreateContentCard($title, $text['summary'], $href);
        }
    }
    $body .= '</div>';
}
...
if (!empty($files)) {
    ...
    foreach ($files as $file) {
        $body .= '<a class="file" href="' . CVUtils\CreateFileHREF($file) . '">';
