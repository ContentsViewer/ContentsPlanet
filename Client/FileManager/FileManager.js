...
class FileManager {
    ...
    static Open() {
        ...
        var form = FileManager.CreateForm({
            'cmd': 'GetFileList', 'directoryPath': fileElement.path,
            'token': fileManager.token, 'pattern': fileManager.fileExtentionPattern
        });
        ...
        request.onload = function (e) {
            ...
            for (var i = 0; i < fileList.length; i++) {
                var file = this.fileElement.fileManager.CreateFileElement(false, fileList[i], !this.fileElement.fileManager.readonly);
        ...
        request.onload = function (e) {
            ...
            for (var i = 0; i < folderList.length; i++) {
                var folder = this.fileElement.fileManager.CreateFileElement(true, folderList[i], !this.fileElement.fileManager.readonly);
    ...
    static Rename() {
        ...
        var form = FileManager.CreateForm({
            'cmd': 'Rename', 'oldName': oldName, 'newName': newName, 'token': fileManager.token
        });
        ...
        request.onload = function (e) {
            ...
            if (FileManager.GetDirectoryName(newName) in this.fileElement.fileManager.fileMap) {
                var newDirectory = this.fileElement.fileManager.fileMap[FileManager.GetDirectoryName(newName)];

                if (newDirectory.listElement != null) {
                    var file = this.fileElement.fileManager.CreateFileElement(this.fileElement.isFolder, newName, true);
    ...
    static Add() {
        ...
        if (fileElement.isFolder || (!fileElement.isFolder && !fileElement.options.hideExtention)) {
            newName = fileElement.directoryName + '/' + fileElement.inputField.value;
        }
        else {
            newName = fileElement.directoryName + '/' + fileElement.inputField.value + FileManager.GetExtention(fileElement.path);
        }
        ...
        request.onload = function (e) {
            ...
            var filePath = this.fileElement.isFolder ? this.parsedResponse.directoryPath : this.parsedResponse.filePath;

            var file = this.fileElement.fileManager.CreateFileElement(this.fileElement.isFolder, filePath, true);
    ...
    static UploadFile(file, fileElement) {
        ...
        request.onload = function (e) {
            ...
            var file = this.fileElement.fileManager.CreateFileElement(this.fileElement.isFolder, this.parsedResponse.filePath, true);
            fileElement.element.parentNode.appendChild(file.element);
