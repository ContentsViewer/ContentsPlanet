...
class Splitter{
    ...
    CreateSliderElement(){
        ...
        slider.addEventListener("mousedown", Splitter.MouseDown, {capture: false});
        slider.addEventListener("touchstart", Splitter.MouseDown, {passive: false, capture: false});
    ...
    RectA(){
        ...
        if(this.direction == Splitter.Direction.Horizontal){
            return new Rect(new Vector2(this.rect.position.x, this.rect.position.y),
    ...
    RectB(){
        ...
        var marginX = this.sliderWidth  / document.documentElement.clientWidth * 100;
        var marginY =  this.sliderWidth  / document.documentElement.clientHeight * 100;

        if(this.direction == Splitter.Direction.Horizontal){
            return new Rect(new Vector2(this.rect.position.x, this.rect.position.y + this.rect.size.y * this.percent / 100.0 + marginY),
                            new Vector2(this.rect.size.x, this.rect.size.y * (100 - this.percent) / 100.0 - marginY));
        }
        else{
            return new Rect(new Vector2(this.rect.position.x + this.rect.size.x * this.percent / 100.0 + marginX, this.rect.position.y),
                            new Vector2(this.rect.size.x * (100 - this.percent) / 100.0 - marginX, this.rect.size.y));
        }
    }
    ...
    Resize(){
        ...
        if(this.direction == Splitter.Direction.Horizontal){
            ...
            this.slider.style.top = this.rect.position.y + this.rect.size.y * this.percent / 100.0 + "%";
            this.slider.style.width = this.rect.size.x + "%";
            this.slider.style.height = this.sliderWidth + "px";
        }
        else{
            this.slider.style.left = this.rect.position.x + this.rect.size.x * this.percent / 100.0 + "%";
    ...
    Split(side, direction, elementToPutEmptyArea, percent, onResizeElementCallbackFunc){
        ...
        var options = {'parent' : this, 'percent' : percent, 'rect' : childRect,
                        'onResizeElementACallbackFunc' :  (side == Splitter.Side.A) ? this.onResizeElementACallbackFunc : this.onResizeElementBCallbackFunc,
                        'onResizeElementBCallbackFunc' : onResizeElementCallbackFunc};

        var childSplitter = new Splitter(direction, 
                                        (side == Splitter.Side.A) ? this.elementA : this.elementB,
                                        elementToPutEmptyArea, options);
    ...
    static MouseDown(e) {
        ...
        //ムーブイベントにコールバック
        document.body.addEventListener("mousemove", Splitter.MouseMove, {capture: false});
        document.body.addEventListener("touchmove", Splitter.MouseMove,  {passive: false, capture: false});
    }
    ...
    static MouseMove(e) {
        ...
        var top = (event.pageY  - drag.fromY) / document.documentElement.clientHeight * 100;
        var left = (event.pageX - drag.fromX) / document.documentElement.clientWidth * 100;
        ...
        if(drag.splitter.direction == Splitter.Direction.Horizontal){
            ...
            drag.style.top = top + "%";
            drag.splitter.percent = (top - drag.splitter.rect.position.y) / drag.splitter.rect.size.y * 100;
            //alert(top);
        }
        else{
            ...
            drag.splitter.percent = (left - drag.splitter.rect.position.x) / drag.splitter.rect.size.x * 100;
        }


        //マウスボタンが離されたとき、またはカーソルが外れたとき発火
        drag.addEventListener("mouseup", Splitter.MouseUp, {capture: false});
        document.body.addEventListener("mouseleave", Splitter.MouseUp, {capture: false});
        drag.addEventListener("touchend", Splitter.MouseUp,  {capture: false});
        document.body.addEventListener("touchleave", Splitter.MouseUp,  {capture: false});
    ...
    static MouseUp(e) {
        ...
        //ムーブベントハンドラの消去
        document.body.removeEventListener("mousemove",Splitter.MouseMove, {capture: false});
        drag.removeEventListener("mouseup", Splitter.MouseUp, {capture: false});

        document.body.removeEventListener("touchmove", Splitter.MouseMove,  {passive: false, capture: false});
        drag.removeEventListener("touchend", Splitter.MouseUp,  {capture: false});
        
        document.body.removeEventListener("mouseleave", Splitter.MouseUp, {capture: false});
        document.body.removeEventListener("touchleave", Splitter.MouseUp,  {capture: false});
